...
func DefaultKeyMap() KeyMap {
	return KeyMap{
		...
		Filter:  key.NewBinding(key.WithKeys("/"), key.WithHelp("/", "filter (running sessions)")),
		History: key.NewBinding(key.WithKeys("h"), key.WithHelp("h", "history")),
	}
}

func (k KeyMap) ShortHelp() []key.Binding {
	return []key.Binding{k.Quit, k.Help, k.Tab, k.Enter, k.Filter, k.History, k.Kill}
}

func (k KeyMap) FullHelp() [][]key.Binding {
	return [][]key.Binding{
		{k.Up, k.Down, k.Enter, k.Escape},
		{k.Tab, k.Filter, k.History},
		{k.Kill, k.Export},
		{k.Help, k.Quit},
	}
}
